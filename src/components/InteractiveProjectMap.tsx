      ...
      <div className="p-6 border-b border-[#e5e1da] flex flex-wrap items-center justify-between gap-4 bg-[#fdfcfb]">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif text-xl sm:text-2xl font-light italic text-[#1a1a1a]">
              Goa Architectural Cartography
            </h3>
          </div>
          <p className="text-xs text-[#8c857d] mt-1 font-light">
        ...
        <div className="lg:col-span-8 relative bg-[#f4f1ee] p-6 sm:p-10 flex items-center justify-center overflow-hidden min-h-[420px] sm:min-h-[540px]">
          ...
          <div className="relative w-full max-w-lg aspect-[3/4] sm:aspect-[4/5] bg-white border border-[#e5e1da] p-6 sm:p-8 shadow-sm flex flex-col justify-between">
            ...
            <div className="flex justify-between items-start text-xs border-b border-[#e5e1da] pb-2">
              <span className="text-[#8c857d] uppercase tracking-widest font-semibold text-[10px]">
                North Goa • Siolim / Assagao / Candolim / Porvorim
              </span>
              <span className="text-[10px] text-[#8c857d] font-mono">Mopa Int'l ↗</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs py-2 border-b border-[#e5e1da] my-auto">
              ...
              <span className="text-[10px] text-[#8c857d] font-mono">Mandovi River 〰</span>
            </div>
            ...
            <div className="flex justify-between items-end text-xs border-t border-[#e5e1da] pt-2">
              ...
              <span className="text-[10px] text-[#8c857d] font-mono">↙ Dabolim Airport</span>
            </div>
            ...
            <div className="absolute top-[8%] right-[10%] text-[10px] text-[#4a4540] bg-[#f4f1ee] px-2 py-0.5 border border-[#e5e1da] pointer-events-none font-mono">
              ✈ Mopa Airport
            </div>
            <div className="absolute top-[52%] right-[12%] text-[10px] text-[#4a4540] bg-[#f4f1ee] px-2 py-0.5 border border-[#e5e1da] pointer-events-none font-mono">
              ✈ Dabolim Airport
            </div>
            <div className="absolute top-[38%] left-[8%] text-[10px] text-[#4a4540] bg-[#f4f1ee] px-2 py-0.5 border border-[#e5e1da] pointer-events-none font-mono">
              🏖 Candolim Shore
            </div>
            ...
            {filteredPins.map((property) => {
              ...
              return (
                <button
                  ...
                  className={`absolute -translate-x-1/2 -translate-y-1/2 group focus:outline-none transition-all duration-300 z-30 ${
                ...
                >
                  ...
                  {/* Pin Tooltip label */}
                  <div
                    className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap px-2.5 py-1 text-[10px] uppercase tracking-widest font-semibold transition-all pointer-events-none shadow-md ${
                      isSelected
          ...
          {selectedPin ? (
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-[#8c857d] flex items-center gap-1">
                    <MapPin className="w-3 h-3 text-[#8c857d]" />
                    {selectedPin.location.area}, {selectedPin.location.region}
                  </p>
                  <h4 className="font-serif text-2xl italic font-normal text-[#1a1a1a] mt-1">{selectedPin.title}</h4>
                </div>

                {/* Specs & Pricing */}
                <div className="bg-[#fdfcfb] p-4 border border-[#e5e1da] space-y-2 text-xs">
                  <div className="flex justify-between items-center">
                    <span className="text-[#8c857d] text-[10px] uppercase tracking-widest">Typology</span>
                    <span className="font-medium text-[#1a1a1a]">{selectedPin.specs.bhk}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[#8c857d] text-[10px] uppercase tracking-widest">Area Size</span>
                    <span className="font-medium text-[#1a1a1a]">{selectedPin.specs.sqftRange}</span>
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t border-[#e5e1da]">
                    <span className="text-[#8c857d] text-[10px] uppercase tracking-widest">Starting Price</span>
                    <span className="font-bold text-[#1a1a1a] font-serif text-base">{selectedPin.price.displayPrice}</span>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedPin.landmarks.map((landmark, idx) => (
                      <div key={idx} className="bg-[#f4f1ee] p-2 border border-[#e5e1da] text-xs">
                        <p className="text-[#1a1a1a] font-medium truncate">{landmark.name}</p>
                        <p className="text-[#8c857d] text-[10px] font-mono mt-0.5">{landmark.distance}</p>
                      </div>
                    ))}
                  </div>
                </div>
